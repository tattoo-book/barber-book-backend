...
export class TattooArtistController {
  static logger = new Logger('TattooArtistController');
  ...
  async create(@Req() req: RequestDTO, @Body() createTattooArtistDto: CreateTattooArtistDTO) {
    try {
      ...
      const errorDescription = ErrorHandler.execute(TattooArtistController.logger, 'Failed on create tattoo artist', error);
